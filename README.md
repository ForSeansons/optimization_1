...
$$
\beta^{k+1} &= \left(X^\top X / n + \rho I\right)^{-1} \left(X^\top y / n + \rho(z^k - u^k)\right) z^{k+1} &= S_{\lambda/\rho}\left(\beta^{k+1} + u^k\right)
$$
...
$$
d^{k+1} &= \frac{d^k + \sigma(X \bar{\beta}^k - y)}{1 + \sigma n} \beta^{k+1} &= S_{\tau\lambda}\left(\beta^k - \tau X^{\top} d^{k+1}\right) \bar{\beta}^{k+1} &= \beta^{k+1} + \theta(\beta^{k+1} - \beta^k)
$$
